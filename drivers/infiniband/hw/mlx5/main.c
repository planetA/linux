...
static int mlx5_ib_stage_send_delay_init(struct mlx5_ib_dev *dev)
{
	...
	debugfs_create_atomic_t("send_delay", 0640, root,
				&dev->delay.send);
	debugfs_create_atomic_t("recv_delay", 0640, root,
				&dev->delay.send);
	debugfs_create_atomic_t("poll_delay", 0640, root,
				&dev->delay.send);
...
{
	if (!dev->delay.dir_debugfs)
		return;

	debugfs_remove_recursive(dev->delay.dir_debugfs);
	dev->delay.dir_debugfs = NULL;
}
