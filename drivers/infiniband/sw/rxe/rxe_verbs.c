				 ...
				 const struct rxe_dump_qp *qp, ssize_t size)
{
	...
	rqp->resp.wqe = queue_head(rqp->rq.queue, QUEUE_TYPE_FROM_USER);
...
static int rxe_restore_mr_keys(struct rxe_mr *mr,
			 ...
			 struct ib_udata *udata)
{
	...
	rxe->qp_pool.index.last = last_qpn;
	...
	last_qpn = rxe->qp_pool.index.last;
...
static struct ib_mr *rxe_get_dma_mr(struct ib_pd *ibpd, int access)
{
	...
	rxe->mr_pool.index.last = last_mrn;

	rxe_add_index(mr);

	last_mrn = rxe->mr_pool.index.last;
