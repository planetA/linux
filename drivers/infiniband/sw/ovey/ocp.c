...
int ocp_fini(void) {
    if (ocp_daemon_sockets_are_known()) {
        ...
        /*struct nlmsghdr * hdr = */ocp_kernel_request_put(ocp_msg, OVEY_C_KERNEL_MODULE_BYE);
        // finalize not needed, because we don't have properties added
        int ocp_ret = ocp_send_kernel_request(ocp_msg);
        if (ocp_ret) {
	...
	int res = genl_unregister_family(&ovey_gnl_family);
	if (res < 0) {
...
int ocp_cb_new_device(struct sk_buff *skb, struct genl_info *info) {
    ...
    ovey_device_info.device_name = ocp_get_string_attribute(info, OVEY_A_VIRT_DEVICE);
    if (!ovey_device_info.device_name) {
        opr_err("received no valid value for OVEY_A_VIRT_DEVICE!\n");
        goto err;
    }
    // we only get a reference; we don't own the data
    ovey_device_info.parent_device_name = ocp_get_string_attribute(info, OVEY_A_PARENT_DEVICE);
    if (!ovey_device_info.parent_device_name ) {
        opr_err("received no valid value for OVEY_A_PARENT_DEVICE!\n");
        goto err;
    }
    ovey_device_info.virt_network_id = ocp_get_string_attribute(info, OVEY_A_VIRT_NET_UUID_STR);
    if (!ovey_device_info.virt_network_id) {
        opr_err("received no valid value for OVEY_A_VIRT_NET_UUID_STR!\n");
        goto err;
    }
    ovey_device_info.node_guid = ocp_get_u64_attribute(info, OVEY_A_NODE_GUID);
    if (!ovey_device_info.node_guid) {
        opr_err("received no valid value for OVEY_A_NODE_GUID!\n");
        goto err;
    }

	parent = ib_device_get_by_name(ovey_device_info.parent_device_name, RDMA_DRIVER_UNKNOWN);
	if (!parent) {
...
int ocp_cb_device_info(struct sk_buff *skb, struct genl_info *info) {
    ...
    device_info.device_name = ocp_get_string_attribute(info, OVEY_A_VIRT_DEVICE);
    ...
    ret = nla_put_string(reply_skb, OVEY_A_VIRT_DEVICE, device_info.device_name);
    if (ret < 0) {
        opr_err("nla_put_string() for OVEY_A_VIRT_DEVICE failed because of %d\n", ret);
        goto err_free;
    }
    ret = nla_put_string(reply_skb, OVEY_A_PARENT_DEVICE, device_info.parent_device_name);
    if (ret < 0) {
        opr_err("nla_put_string() for OVEY_A_PARENT_DEVICE failed because of %d\n", ret);
        goto err_free;
    }
    ret = nla_put_be64(reply_skb, OVEY_A_NODE_GUID, device_info.node_guid, 0);
    if (ret < 0) {
        opr_err("nla_put_string() for OVEY_A_NODE_GUID failed because of %d\n", ret);
        goto err_free;
    }
    ret = nla_put_be64(reply_skb, OVEY_A_PARENT_NODE_GUID, device_info.parent_node_guid, 0);
    if (ret < 0) {
        opr_err("nla_put_string() for OVEY_A_PARENT_NODE_GUID failed because of %d\n", ret);
        goto err_free;
    }
    ret = nla_put_string(reply_skb, OVEY_A_VIRT_NET_UUID_STR, device_info.virt_network_id);
    if (ret < 0) {
...
int ocp_cb_daemon_hello(struct sk_buff *skb, struct genl_info *info) {
    ...
    received_socket_kind_attribute = ocp_get_u32_attribute(info, OVEY_A_SOCKET_KIND);

    opr_info("OCP-request: OVEY_C_DAEMON_HELLO\n");
    opr_info("    sending_socket_port_id (source socket id)    =%d\n", sending_socket_port_id);
    opr_info("    netlink_hdr_port_id (source socket id)=%d (OcpKindSocket::%s)\n",
             netlink_hdr_port_id,
             ocp_socket_kind_to_string(netlink_hdr_port_id)
    );
    opr_info("    received socket kind attribute        =%d (OcpKindSocket::%s)\n",
    ...
    if (netlink_hdr_port_id == KERNEL_INITIATED_REQUESTS_SOCKET) {
        ...
        opr_err("netlink_hdr_port_id = %s is not a valid value", ocp_socket_kind_to_string(netlink_hdr_port_id));
        ocp_reply_with_error(info, -EINVAL);
        return -1;
    }
...
int ocp_cb_debug_resolve_all_completions(struct sk_buff *skb, struct genl_info *info) {
    ...
    list_for_each_entry_safe(curr, n, &ovey_completion_list.list_item, list_item) {
        if (!curr->completion_resolved) {
            opr_info("Entry with completion_id=%lld not resolved yet: resolving now\n", curr->req_id);
            ovey_completion_resolve_by_id(curr->req_id);
        } else {
            opr_info("Entry with completion_id=%lld already resolved\n", curr->req_id);
        }
    }
...
int ocp_daemon_sockets_are_known(void) {
    return ocp_sockets.genl_sock != NULL && ocp_sockets.kernel_daemon_to_sock_pid != -1 && ocp_sockets.daemon_to_kernel_sock_pid != -1;
};
