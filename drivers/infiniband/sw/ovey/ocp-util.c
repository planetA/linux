...
void ocp_reply_with_error(struct genl_info *info, int err_code) {
    ...
    nlmsghdr = nlmsg_put(reply_skb, info->snd_portid, 0, NLMSG_ERROR, payload_size, 0);
    if (nlmsghdr == NULL) {
    ...
    rc = nlmsg_unicast(genl_info_net(info)->genl_sock, reply_skb, info->snd_portid);
...
struct nlmsghdr *ocp_kernel_request_put(struct sk_buff * req_sk_buf, enum OveyOperation cmd) {
    struct nlmsghdr * hdr = genlmsg_put(
            req_sk_buf,
