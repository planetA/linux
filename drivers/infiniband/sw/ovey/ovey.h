...
/* We allow only one port */
#define OVEY_PHYS_PORT_CNT 1
...
struct ovey_device {
	...
	// For debugging purposes we may want to use real ids instead of virtual ones
	atomic_t real_guid;
	atomic_t real_gid;
	atomic_t real_lid;
	atomic_t real_qpn;
 ...
 */
	struct ovey_device_info *get_device_info_by_name(
		char const *const ovey_dev_name, struct ovey_device_info *dest);
...
extern const struct attribute_group ovey_attr_group;
